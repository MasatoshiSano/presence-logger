...
**デスクトップの「子をこのハブへ付ける」を使う（推奨）**

ブラウザの fleet-ui（`:8090`）は使わない。ハブのデスクトップアイコンから質問に答える。
間違えたら `0` で直前の質問に戻れる（漢字の「戻る」でも可）。

- **すでに動いている子を移す**: 名前と局番号はそのまま
- **新しい子を増やす**: クローンして改名する。局番号は空になる

STA_NO の設定は子の Web UI（`:8080`）で行う。ハブ側ウィザードはそこまでは面倒を見ない。
