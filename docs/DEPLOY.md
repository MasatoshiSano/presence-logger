...
対応付けできない子（電源が落ちている等）がある間は、**登録候補を出さない**。
稼働中の子がIPドリフトで「未登録の端末」に見えることがあり、それを登録すると
本番機を再プロビジョニングしてしまうため。画面に理由が出るので、先に既存の子を
復旧するかインベントリから外す。

STA_NO の設定（手順9）は子の Web UI で行う。UI はそこまでは面倒を見ない。

初回だけ常駐サービスを導入する（親側で sudo が要るのはここだけ）:
...
ss -ltn | grep 8090        # 127.0.0.1:8090 のみであることを確認
```
