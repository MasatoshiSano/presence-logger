...
- Produces:
  ...
  - `run_cmd(cmd: list[str]) -> str` — 外部コマンドを実行し標準出力を返す。失敗しても例外にせず空文字。
    Task 3/4 がこれを既定の runner として使う
  - `read_neighbors(*, dev='wlan1', runner=...) -> list[Neighbor]`
  - `AP_DEV: str` — `"wlan1"`
...
def read_neighbors(*, dev: str = AP_DEV, runner: Callable[[list[str]], str] = run_cmd) -> list[Neighbor]:
...
```bash
git add fleet_ui/collect.py fleet_ui/tests/test_collect.py
...
Run: `.venv/bin/python -m pytest fleet_ui/tests/ -q`
Expected: 46 passed（Task 1-4 の39件 + 本タスクの7件）
...
Expected: 既存の全テスト + fleet_ui の46件がすべて成功。
