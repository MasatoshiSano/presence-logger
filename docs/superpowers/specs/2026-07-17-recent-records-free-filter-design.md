...
「種別で絞る」は入室/退室の2択ではなく**任意コード指定**にする。表示側も入退室への
翻訳をやめ、`T1_STATUS` は**数字そのまま**で見せる（詳細は ③）。
...
- **`T1_STATUS` は入退室(🟢ENTER/🔴EXIT)に翻訳せず、数字そのままで表示する。**
  理由: 子Piから来るデータは入退室(1/2)に限らず `3,11,14,17,99` など任意の状態
  コードを持つため、1/2 だけを特別扱いする翻訳は誤解を招く。全コードを生値で
  一様に見せる。
  - 実装: 現行の `STATUS = {"1":"🟢ENTER","2":"🔴EXIT "}` マッピングと凡例を撤去し、
    列見出しは「種別」→「T1」等にし、値は `t1` をそのまま出す。
- 適用フィルタの見出し追記のみ（シェル側で出すなら不要）。
