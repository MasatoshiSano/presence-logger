...
4. 終わったら再起動する。
   - **既存の子**（親が動いている / 親が無い / SDクローン）: デスクトップの
     **「子をこのハブへ付ける」** をクリックし、質問に答える。ホスト名と局番号が残る。
     登録ウィザードは使わない。
   - **新しい子Pi / 同じハブへのクローン増設**: 新しい AP に繋いでから **フリート管理** の
     登録ウィザード（局番号を空にして改名）
...
> デスクトップの **「子をこのハブへ付ける」** がターミナルで順に聞く。
>
...
>
> 同じ操作はフリート管理の画面からもできる。AP パスワードはブラウザへ送らない。
>
