...
4. 終わったら再起動する。
   ...
   - **親機に付いている子**: 登録ウィザードは使わない。フリート管理の
...
> フリート管理の **「他のハブから引き継ぐ」** が、工場網経由で旧親へ SSH し、公開鍵の追加・
> 新 AP への WiFi 切替・新ハブの `children.conf` 追加・旧親からの削除を行う。先に新ハブから
> 旧親へ一度だけ `ssh-copy-id` しておく（画面に公開鍵とコマンドが出る）。AP パスワードは
> ブラウザへ送らない。ハブ初期設定が書く `.kit/ap-join.env`（`pi` が読める 600）が必要。
>
...
- **既存の子が1台でも到達不能だと、フリート管理に登録候補が出ない。** これは仕様である
  ...
  しないための保護）。引っ越し中は **「他のハブから引き継ぐ」** を使い、登録ウィザードは
  使わない。
