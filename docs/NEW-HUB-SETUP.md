...
4. 終わったら再起動する。
   - **新しい子Pi**: 新しい AP に繋いでから **フリート管理** の登録ウィザードで追加する
   - **親機が同じ工場網でまだ動いている子**: 登録ウィザードは使わない。フリート管理の
     **「他のハブから引き継ぐ」** で移す（ホスト名と局番号が残る）
   - **親機がもう無い・別工場 / 子のSDをクローンした**: デスクトップの
     **「子SDをこのハブ用にする」** で鍵と AP を書いてから起動し、フリート管理で
     **「名前と局番号を残して取り込む」**。登録ウィザードは使わない
...
> パスワードを聞くことすらしない）。
>
> **親機の子を新ハブへ移す**ときは、新機登録ウィザード（STA_NO を空にする / 改名）を使わない。
>
> - **旧親が同じ工場網でまだ動いている**: フリート管理の **「他のハブから引き継ぐ」** が、
>   工場網経由で旧親へ SSH し、公開鍵の追加・新 AP への WiFi 切替・インベントリの付け替えを行う。
>   先に新ハブから旧親へ一度だけ `ssh-copy-id` しておく。
> - **旧親が止まっている・別工場網 / 子のSDをクローンした**: 入れ子 SSH は使えない。
>   新ハブでデスクトップ **「子SDをこのハブ用にする」**（`scripts/prepare-child-sd.sh`）が、
>   マウントした子の rootfs へこのハブの公開鍵と AP 参加情報を書く。ホスト名と局番号は触らない。
>   子を起動してこの AP に現れたら、フリート管理で **「名前と局番号を残して取り込む」**。
> - ハブ初期設定で「親機はもう使わない／別工場」を選ぶと、親と同じ AP 名を許可する
>   （`ORIGIN_ALLOW_SAME_AP=1`）。クローンした子が自動で付く。鍵は上の SD 書き込みが必要。
>
> AP パスワードはブラウザへ送らない。ハブ初期設定が書く `.kit/ap-join.env`（`pi` が読める 600）が必要。
>
...
- **既存の子が1台でも到達不能だと、フリート管理に登録候補が出ない。** これは仕様である
  ...
  しないための保護）。引っ越し中は **「他のハブから引き継ぐ」** か、旧親が無いなら
  **「子SDをこのハブ用にする」→「名前と局番号を残して取り込む」** を使い、
  登録ウィザードは使わない。
