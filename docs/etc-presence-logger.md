...
>
> **実設定のバックアップ**: この拠点の非秘密の実設定（本番値）は
> [`../config/site/`](../config/site/) にスナップショットしてあり、移行・復元の元になります
> （`config/*.example` は汎用テンプレート、`config/site/*` がこの拠点の実構成）。
...
```yaml
profiles:
  taden-ot-ap:                       # キー = 接続する SSID 名
    description: "..."
    wifi:                            # 任意: 運用スクリプト用（bridge は読まない）
      psk: "${WIFI_PSK_TADEN}"       # ← secrets.env から展開
      hidden: false
      static_ipv4:
        address: "172.29.1.4/24"
        gateway: "172.29.1.254"
        dns: ["192.168.250.1"]
    # station: は省略 → device.yaml の station がそのまま使われる
    sntp:
      servers: ["192.168.250.1"]     # この SSID のときだけ到達できる工場内NTP
    oracle:
      client_mode: "jdbc"            # thin | thick | jdbc
      auth_mode:   "basic"           # basic | wallet
      host: "10.168.252.16"; port: 1521; service_name: "HHS001"
      user: "ZHH001"
      password: "${ORACLE_PASSWORD_ONPREM}"  # ← secrets.env から展開
      table_name: "HF1RCM01"
...
ORACLE_PASSWORD_ONPREM=...   # taden-ot-ap の Oracle(HHS001)
...
WIFI_PSK_TADEN=...           # taden-ot-ap の WiFi PSK（接続スクリプトが root で読む）
```
**2系統の読まれ方:**
...
- **WiFi PSK**: コンテナではなく**接続スクリプト**（[`desktop/presence-tools/connect-taden-ot-ap.sh`](../desktop/presence-tools/connect-taden-ot-ap.sh)）が
  実行時に root で読み、`${WIFI_PSK_TADEN}` を展開して `nmcli` に直接渡す（画面・履歴に出さない）。
