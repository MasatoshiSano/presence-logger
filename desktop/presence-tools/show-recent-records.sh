...
resolve_sta() {  # $1=入力 $2=既定（recent_default_sta_no。* は絞らない）
    case "${1:-$2}" in
        "*") printf '' ;;
        *)   printf '%s' "${1:-$2}" ;;
    esac
...
main() {
    ...
    PROFILE_NAME="${PROFILE_NAME:-${FACTORY_SSID:-HIME-H-REAP}}"
...
PY
    ...
    DEF_S1="$(recent_default_sta_no sta_no1 "${PCFG[sta_no1]}")"
    DEF_S2="$(recent_default_sta_no sta_no2 "${PCFG[sta_no2]}")"
    DEF_S3="$(recent_default_sta_no sta_no3 "${PCFG[sta_no3]}")"

    # --- 対話式フィルタ入力（Enter=既定/絞らない、* =その列を絞らない）---
    echo
    echo "  フィルタ条件を入力（Enter で既定 / 詳細は各行の括弧）"
    read -rp "  STA_NO1（Enter=${DEF_S1} / *=すべて）: " IN_S1
    read -rp "  STA_NO2（Enter=${DEF_S2} / *=すべて）: " IN_S2
    read -rp "  STA_NO3（Enter=${DEF_S3} / *=すべて）: " IN_S3
    ...
    F_S1="$(resolve_sta "$IN_S1" "$DEF_S1")"
    F_S2="$(resolve_sta "$IN_S2" "$DEF_S2")"
    F_S3="$(resolve_sta "$IN_S3" "$DEF_S3")"
    F_FROM="$IN_FROM"
    F_TO="$IN_TO"
    F_T1="$IN_T1"
