...
# 工場WiFi「taden-ot-ap」に接続して、繋ぎっぱなしで記録を続ける。
#   - 自動接続はしない（このファイルを実行したときだけ繋ぐ）
#   - 60秒で戻る live テスト(live_taden_ot_ap_run.sh)とは違い、自動で元に戻さない
#   - 戻すときは Desktop の disconnect-taden-ot-ap.sh を実行
#
...
CONN_NAME="${CONN_NAME:-taden-ot-ap}"                # nmcli 接続名（liveの "-live" とは別）
IFNAME="${IFNAME:-wlan0}"
...
# delete + add だと NM が AP の位置を忘れて初回 scan 待ちで up がタイムアウトする
...
# --- 接続（最大60秒待つ）---
say "接続中..."
if nmcli --wait 60 connection up "$CONN_NAME" >/dev/null 2>&1; then
    ...
    if [[ "$ACTUAL" == "$PROFILE_NAME" ]]; then
        ...
        # 自動起動を無効化したので PC 再起動直後は土台も止まっているが、
        # これで「接続」ボタンだけで全コンテナが立ち上がる。
...
say ""
say " ❌ taden-ot-ap に繋がりませんでした（電波圏外の可能性）。"
if [[ -n "$PREV" ]]; then
    say "    元の接続「$PREV」に戻します..."
