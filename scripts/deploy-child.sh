...
#
# 使い方:
#   scripts/deploy-child.sh                 # code + config を配布（既定）
#   scripts/deploy-child.sh --code-only     # コードのみ（config は現状維持）
#   scripts/deploy-child.sh --dry-run       # 何を送るかだけ表示（変更なし）
#   scripts/deploy-child.sh --no-rollback   # 失敗しても自動復元しない
#
# 環境変数で上書き可: CHILD_SSH, CHILD_AP_IP, CHILD_WEB_PORT など（deploy-common.sh 参照）
...
CODE_ONLY=0; DRY_RUN=0; ROLLBACK=1
...
[ "$CODE_ONLY" -eq 1 ] || files+=("${CHILD_CONFIG_FILES[@]}")

# ローカルに全ファイルが揃っているか
for f in "${files[@]}"; do [ -f "$CHILD_SRC/$f" ] || die "child/$f が無い"; done

TS="$(date +%Y%m%d-%H%M%S)"
log "子アプリ配布 (tag=$TS, files=${#files[@]}, code_only=$CODE_ONLY, dry_run=$DRY_RUN)"
