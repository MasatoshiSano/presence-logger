...
### taden-ot-ap (HHS001) 用の `client_mode: jdbc` 経路

旧パスワード検証子 (10G verifier, `0x939`) を残したまま運用されている DB
(HHS001 の `ZHH001` など) は、`python-oracledb` の Thin mode が
...
```yaml
profiles:
  taden-ot-ap:                                     # 工場内 WiFi の SSID
    description: "Factory internal Wi-Fi -> HHS001 via JDBC sidecar"
    sntp:
      servers: ["192.168.250.1"]                   # 社内 NTP（taden-ot-ap は閉域網）
    oracle:
...
Wi-Fi 側の前提 (regdomain JP / 静的IP 172.29.1.4 / GW 172.29.1.254 等) は
`scripts/verify_taden_ot_ap_oracle.sh` 内のコメントを参照。本番投入前に
このスクリプトを一度走らせ、`MERGE` が `rows_affected=1, ora_code=` で
帰ってくることを確認する。スクリプトは `trap cleanup EXIT` で
temp プロファイルを down するため、SSH ごしや Claude セッションが
切れても工場WiFi に置き去りにならない（HOME_PROFILE 環境変数を
設定すれば指定 WiFi へ自動復帰させることもできる）。
